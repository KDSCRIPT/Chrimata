from helper import call_gemini_api

def identify_team_specific_workflows(business_context, inputs):
    """Step 2: Streamlined Workflow Discovery using AI Inference (Merged Prompt)"""
    print("\n--- Step 2: Workflow Discovery by Department (Optimized) ---")
    all_tasks = []
    departments = business_context.get('departments', [])

    if not departments:
        print("No departments found. Please complete Step 1 correctly.")
        return []

    for dept in departments:
        print(f"\n🔍 Exploring {dept} Department...")
        team_summary = f"team_summaries[{dept}]"

        if not team_summary:
            print(f"⚠️ No team summary found for {dept} in inputs.py. Skipping.")
            continue

        # --- Merged Gemini Prompt for both task extraction and AI suggestions ---
        merged_prompt = (
            f"The '{dept}' department provided this summary of their daily work:\n"
            f"\"\"\"\n{team_summary}\n\"\"\"\n\n"
            "From this, extract up to 3 recurring or critical workflows. For each, also suggest an AI or automation improvement.\n"
            "Return an array of JSON objects with the following:\n"
            "- task_name\n"
            "- frequency (Daily, Weekly, Monthly, As needed, Other)\n"
            "- characteristics (e.g. manual, repetitive, prone to delay)\n"
            "- tools_used\n"
            "- dependencies (other departments or 'None')\n"
            "- ai_solution_summary (1-line idea)\n"
            "- ai_tools_or_techniques (e.g., RPA, LLMs, ML Forecasting)\n"
            "- expected_impact (Low / Medium / High)\n"
            "- notes (optional)"
        )

        merged_schema = {
            "type": "ARRAY",
            "items": {
                "type": "OBJECT",
                "properties": {
                    "task_name": {"type": "STRING"},
                    "frequency": {"type": "STRING", "enum": ["Daily", "Weekly", "Monthly", "As needed", "Other"]},
                    "characteristics": {"type": "STRING"},
                    "tools_used": {"type": "STRING"},
                    "dependencies": {"type": "STRING"},
                    "ai_solution_summary": {"type": "STRING"},
                    "ai_tools_or_techniques": {"type": "STRING"},
                    "expected_impact": {"type": "STRING", "enum": ["Low", "Medium", "High"]},
                    "notes": {"type": "STRING"}
                },
                "required": [
                    "task_name", "frequency", "characteristics", "tools_used",
                    "dependencies", "ai_solution_summary",
                    "ai_tools_or_techniques", "expected_impact"
                ]
            }
        }

        results = call_gemini_api(
            merged_prompt,
            schema=merged_schema,
            instruction_type=f"Task + AI inference for {dept}"
        )

        if not results:
            print(f"⚠️ Couldn’t infer structured tasks for {dept}.")
            continue

        for task_data in results:
            task_data['team'] = dept
            task_data['ai_insights'] = {
                "ai_solution_summary": task_data.pop("ai_solution_summary"),
                "ai_tools_or_techniques": task_data.pop("ai_tools_or_techniques"),
                "expected_impact": task_data.pop("expected_impact"),
                "notes": task_data.pop("notes", "")
            }
            all_tasks.append(task_data)
            print(f"✅ {dept} task identified: {task_data['task_name']}")

    if not all_tasks:
        print("No tasks identified. Consider retrying with better descriptions.")
        return None

    print("\n📋 Summary of Identified Tasks:")
    for i, task in enumerate(all_tasks, 1):
        print(f"{i}. [{task['team']}] {task['task_name']} ({task['frequency']})")
        insights = task.get('ai_insights', {})
        if insights:
            print(f"   🔍 {insights.get('ai_solution_summary')} (Impact: {insights.get('expected_impact')})")

    return all_tasks
